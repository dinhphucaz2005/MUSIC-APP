      ...
      if (event is Map) {
        ...
        switch (type) {
          ...
          case 'add':
            ...
            subject.add(LocalSongRepositoryState(songs: [...currentState.songs, song], isLoading: currentState.isLoading));
            break;
          case 'remove':
            final id = event['id'] as String?;
            if (id != null) {
              final updatedSongs = currentState.songs.where((s) => s.path != id).toList();
              subject.add(LocalSongRepositoryState(songs: updatedSongs, isLoading: currentState.isLoading));
            }
            break;
          case 'finish':
            subject.add(LocalSongRepositoryState(songs: currentState.songs, isLoading: false));
            break;
        }
      }
    });
